...
public class TextToSpeechService
{
    ...
    public TextToSpeechService(ILogger<TextToSpeechService> logger, IOptions<OpenAIOptions> openAIOptions)
    {
        ...
        _audioClient = new AudioClient(options.SpeechModelId, options.ApiKey);
    }
    ...
    public async Task<SpeechEvent> TransformAsync(ChatEvent evt) =>
        new SpeechEvent(evt.TurnId, evt.CancellationToken, await SynthesizeAsync(evt.Payload, evt.CancellationToken));
    ...
    private Task<byte[]> SynthesizeAsync(string text, CancellationToken token) =>
        Tools.ExecutePipelineOperationAsync(
            operation: async () =>
            {
                BinaryData speech = await _audioClient.GenerateSpeechAsync(text, DefaultSpeechVoice, null, token);
